...
from infinity.utils.slerp import slerp
...
from infinity.models.bitwise_self_correction import BitwiseConvert
import math
...
class Infinity(nn.Module):
    def __init__(
        ...
        if prefix_config is not None:
            self.special_token = nn.Parameter(torch.zeros(prefix_config.ti_seq_len, 2048))
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        cfg_infer=False, cond_BD=None, cond_BD_or_gss=None, ca_kv=None,
        **kwargs,
        ...
        # x_BLC_wo_prefix : [2, 10520, 32]

        reg_path = False
        # [1. get input sequence x_BLC]
        with torch.amp.autocast('cuda', enabled=False):
            if (cond_BD is not None and cond_BD_or_gss is not None and ca_kv is not None):
                reg_path = True
                x_BLC = x_BLC_wo_prefix
            else:
        ...
        checkpointing_full_block = self.checkpointing == 'full-block'
        if self.num_block_chunks == 1:
    ...
    @torch.no_grad()
    def autoregressive_cls_infer(
        self, vae, label_B_or_BLT, scale_schedule,
        B=1, cfg_list=[], tau_list=[], top_k=0, top_p=0.0,
        **kwargs,
    ):
        ...
        ca_kv = kv_compact, cu_seqlens_k, max_seqlen_k
        ca_kv_ = kv_compact.clone().detach(), cu_seqlens_k, max_seqlen_k
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            x_BLC_list.append(last_stage.clone().detach())
            for chunk in self.block_chunks:
                last_stage = self.add_lvl_embeding(last_stage, si, scale_schedule, need_to_pad=0)
                last_stage = chunk(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv,
            ...
            ori_logits_BLV.append(logits_BlV.clone().detach())
            logits_BlV = logits_BlV.mul(1/tau_list[si])
            ...
            idx_Bld_list.append(idx_Bld.unsqueeze(1))
        ...
        ori_logits_BLV = torch.cat(ori_logits_BLV, dim=1)
        x_BLC = torch.cat(x_BLC_list, dim=1)
        return x_BLC, ori_logits_BLV, cond_BD.clone().detach(), cond_BD_or_gss.clone().detach(), ca_kv_
    ...
    def autoregressive_infer_cfg(
        ...
        vae_type=32, softmax_merge_topk=-1, ret_img=False,
        trunk_scale=1000,
        gt_leak=0, gt_ls_Bl=None,
        inference_mode=False,
        save_img_path=None,
        sampling_per_bits=1,
        return_x_BLC_wo_prefix=False,
        training=False,
        save_img=True,
    ):   # returns List[idx_Bl]
        ...
        gt_logits_BLVs = []
        gt_logits_BLVs_wo_inf = []
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            cfg = cfg_list[si]
            if si >= trunk_scale:
                break
            # if si == self.init_scale - 1:
            #     last_stage = self.init_token.clone().unsqueeze(0).expand(bs, -1, -1)
            cur_L += np.array(pn).prod()
            ...
            gt_logits_BLVs.append(logits_BlV)
            gt_logits_BLVs_wo_inf.append(logits_BlV.clone())
            # print(torch.isinf(logits_BlV).any())
            # print(logits_BlV.shape)
            # print(logits_BlV)
            # print(gt_logits_BLVs[0])

            # sample会将logit值变为-inf
            ...
            if vae_type != 0:
                ...
                if si != num_stages_minus_1:
                    ...
                    last_stage = last_stage.reshape(*last_stage.shape[:2], -1) # [B, d, h*w] or [B, 4d, h*w]
                    last_stage = torch.permute(last_stage, [0,2,1]) # [B, h*w, d] or [B, h*w, 4d]
                    x_BLC_wo_prefix.append(last_stage)
                else:
                    summed_codes += codes
            else:
                if si < gt_leak:
        ...
        if training and not save_img:
            x_BLC_wo_prefix = torch.cat(x_BLC_wo_prefix, 1)
            return gt_logits_BLVs, x_BLC_wo_prefix, 
            
        if return_x_BLC_wo_prefix:
            x_BLC_wo_prefix = torch.cat(x_BLC_wo_prefix, 1)
        ...
        gt_ms_idx_Bl = [item.reshape(B, -1, 32) for item in idx_Bld_list]
        
        if training:
            # for i in range(len(gt_logits_BLVs)):
            #     print(torch.isinf(gt_logits_BLVs_wo_inf[i]).any())
                # print(gt_logits_BLVs[i].shape)
            return gt_logits_BLVs, x_BLC_wo_prefix, img, gt_ms_idx_Bl, gt_logits_BLVs_wo_inf
...
def sample_with_top_k_top_p_also_inplace_modifying_logits_(logits_BlV: torch.Tensor, top_k: int = 0, top_p: float = 0.0, rng=None, num_samples=1) -> torch.Tensor:  # return idx, shaped (B, l)
    ...
    # [1, 32*h*w, 2]
    # print('1', logits_BlV)
    # masked_fill_会将logits_BlV值变为-inf
    if top_k > 0:
        # top_k= 900, V = 2
        # 获得最高的两个logit然后移除所有小于这个值的logit，所以实际上这一步不会产生-inf
        top_k = min(top_k, V)
        # print(top_k)
        # print(logits_BlV.topk(top_k, largest=True, sorted=False, dim=-1)[0].amin(dim=-1, keepdim=True))
        # print(logits_BlV)
    ...
    if top_p > 0:
        # top_p = 0.97
        # print(logits_BlV)
        # print(logits_BlV.reshape(B, -1, 64).softmax(dim=-1))
        sorted_logits, sorted_idx = logits_BlV.sort(dim=-1, descending=False)
        # print(sorted_logits.softmax(dim=-1).cumsum_(dim=-1))
    ...
    # print('3', logits_BlV[0][0], probs[0][0])
    # print('3', logits_BlV)
    # 检查是否有无效概率
