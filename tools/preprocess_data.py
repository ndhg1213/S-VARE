...
def gen_one_img(
    ...
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16, cache_enabled=True):
        stt = time.time()
        gt_logits_BLVs, x_BLC_wo_prefix, img_list, gt_ms_idx_Bl, gt_logits_BLVs_wo_inf = infinity_test.autoregressive_infer_cfg(
            vae=vae,
            ...
            training=True, save_img=True,
        )
    print(f"cost: {time.time() - sstt}, infinity cost={time.time() - stt}")
    generated_image = img_list[0]
    return gt_logits_BLVs, x_BLC_wo_prefix, generated_image, gt_ms_idx_Bl, gt_logits_BLVs_wo_inf
...
def save_slim_model(infinity_model_path, save_file=None, device='cpu', key='gpt_fsdp'):
    ...
    try:
        ...
        infinity_slim = full_ckpt['trainer']['gpt_wo_ddp']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json_file', type=str, default='./inputs/train_church.json')
    parser.add_argument('--save_path', type=str, default='./inputs/train_church_data')
    ...
    os.makedirs(os.path.join(save_path, 'target', 'label'), exist_ok=True)
    os.makedirs(os.path.join(save_path, 'target', 'image'), exist_ok=True)
    with autocast(dtype=torch.bfloat16):
        with torch.no_grad():
            for idx, data in tqdm(enumerate(datas), total=len(datas)):
                prompt = data['modified_prompt']
                idx = data['idx']
                gt_logits_BLVs, x_BLC_wo_prefix, generated_image, gt_ms_idx_Bl, gt_logits_BLVs_wo_inf = gen_one_img(
                    infinity,
                    vae,
                    text_tokenizer,
                    text_encoder,
                ...
                # 构造保存文件名，包含idx以区分不同样本
                
                save_file = os.path.join(save_path, 'target', 'label', f'{idx}.pt')
                img_file = os.path.join(save_path, 'target', 'image',f'{idx}.png')
                success = cv2.imwrite(img_file, generated_image.cpu().numpy())
                # 保存为pt文件
                torch.save({
    ...
    os.makedirs(os.path.join(save_path, 'source', 'label'), exist_ok=True)
    os.makedirs(os.path.join(save_path, 'source', 'image'), exist_ok=True)          
    with autocast(dtype=torch.bfloat16):
        with torch.no_grad():
            for idx, data in tqdm(enumerate(datas), total=len(datas)):
                ...
                save_file = os.path.join(save_path, 'source', 'label', f'{idx}.pt')
                img_file = os.path.join(save_path, 'source', 'image',f'{idx}.png')
                success = cv2.imwrite(img_file, generated_image.cpu().numpy())
                torch.save({
                    'gt_logits_BLVs': gt_logits_BLVs,
                    'x_BLC_wo_prefix': x_BLC_wo_prefix, 
