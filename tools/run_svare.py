...
def encode_prompt(text_tokenizer, text_encoder, prompt, enable_positive_prompt=False):
    ...
    captions = [prompt]
    tokens = text_tokenizer(text=captions, max_length=512, padding='max_length',
...
def load_infinity(
    ...
    with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16, cache_enabled=True), torch.no_grad():
        ...
        state_dict = torch.load(custom_model_path, map_location=device)['model']
...
def joint_vi_vae_encode_decode(vae, image_path, scale_schedule, device, tgt_h, tgt_w):
    ...
    # print(recons_img.shape, gt_img.shape) # [1, 512] , [1, 512]
...
def add_common_arguments(parser):
    ...
    parser.add_argument('--text_prefix_length', type=int, default=4)
    parser.add_argument('--vae_path', type=str, default='../../Models/Infinity/infinity_vae_d32_reg.pth')
    ...
    parser.add_argument('--text_encoder_ckpt', type=str, default='../../Models/flan-t5-xl')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_file', type=str, default='local_output.jpg')
    parser.add_argument('--num_samples', type=int, default=1)
    parser.add_argument('--result_name', type=str, default='output.jpg')
    ...
    with torch.amp.autocast('cuda', dtype=torch.bfloat16):
        with torch.no_grad():
            for i in range(args.num_samples):
                ...
                cv2.imwrite(osp.join(args.result_dir, args.result_name), generated_image.cpu().numpy())
                print(f"Save to {osp.abspath(osp.join(args.result_dir, args.result_name))}")
